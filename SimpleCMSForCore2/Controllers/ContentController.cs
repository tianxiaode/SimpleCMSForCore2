...
using SimpleCMSForCore2.Controllers;
...
namespace SimpleCMSForCore2.Controllers
{
    ...
    public class ContentController : BaseController
    {
        ...
        public ContentController(ApplicationDbContext context, SignInManager<ApplicationUser> signInManager,
        ...
        {
            var q = DbContext.Contents.Include(m => m.Category).Include(m => m.ContentTags).Where(m => m.State == 0);
            ...
            foreach (var c in q)
            {
                ja.Add(new JObject()
                {
                    ...
                    { "Hits",  c.Hits},
                    { "Tags",  JArray.FromObject(c.ContentTags.Select(m=>m.Tag.Name)) }
                });
            }
        ...
        public async Task<JObject> Details(int? id)
        {
            ...
            var q = await DbContext.Contents.Include(m => m.Category).Include(m => m.ContentTags).SingleOrDefaultAsync(m => m.Id == id && m.State == 0);
            ...
            return ExtJs.WriterJObject(true, data: new JObject()
            {
                ...
                { "Tags",  string.Join(",",q.ContentTags.Select(m=>m.Tag.Name).ToList())}
            });
        ...
        public async Task<JObject> Create(ContentModel model)
        {
            ...
            var record = new Content()
            {
                ...
                //ContentTags = DbContext.Tags.Where(m => model.Tags.Contains(m.Name)).ToList()
            };
            foreach (var c in DbContext.Tags.Where(m => model.Tags.Contains(m.Name)))
            {
                record.ContentTags.Add(new ContentTag()
                {
                    Content = record,
                    Tag = c
                });
            ...
            return ExtJs.WriterJObject(true, data: new JObject()
            {
                ...
                { "Tags",  string.Join(",",record.ContentTags.Select(m=>m.Tag.Name).ToList())}
            });
        ...
        public async Task<JObject> Update(ContentModel model)
        {
            ...
            foreach (var c in DbContext.Tags.Where(m => model.Tags.Contains(m.Name)))
            {
                q.ContentTags.Add(new ContentTag()
                {
                    Content = q,
                    Tag = c
                });
