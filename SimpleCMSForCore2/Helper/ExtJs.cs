...
namespace SimpleCMSForCore2.Helper
{
    public class ExtJs
    {
        ...
        public static JObject ModelStateToJObject(ModelStateDictionary modelState)
        {
            ...
            var q = modelState.Where(m => modelState.GetFieldValidationState(m.Key) != ModelValidationState.Valid).Select(m => m.Key);
            foreach (var c in q)
            {
        ...
        public static IQueryable<T> OrderBy<T>(string sortStr, JObject allowSorts, IQueryable<T> queryable)
        {
            ...
            if (first == null || string.IsNullOrEmpty((string)first.Value)) throw new HttpListenerException(500, Message.NoAllowSortDefine);
